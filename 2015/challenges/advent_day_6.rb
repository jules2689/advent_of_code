...
module Advent
  class Day6 < Base
    ...
    def challenge_1(challenge_input=input)
      run_challenge challenge_input, 1 do |pos_value, modifier|
        modifier > -1 ? modifier : (pos_value == 1 ? 0 : 1)
      end
    end

    def challenge_2(challenge_input=input)
      run_challenge challenge_input, 2 do |pos_value, modifier|
        [pos_value + modifier, 0].max
      end
    end

    def run_challenge(challenge_input, challenge, &block)
      lights = Array.new(1000) { Array.new(1000, 0) }

      challenge_input.each do |instruction|
        instruction_matches = instruction.scan(/\d*,\d*/)
        initial = instruction_matches.first.split(",").map(&:to_i)
        final = instruction_matches.last.split(",").map(&:to_i)
       
        modifier = determine_modifier(instruction, challenge)

        for i in initial.first .. final.first
          for j in initial.last .. final.last
            lights[i][j] = block.call(lights[i][j], modifier)
          end
        end
      end

      lights.map { |light_row| light_row.inject(&:+) }.inject(&:+)
    end

    def determine_modifier(instruction, challenge)
    ...
    def test
      perform_test(1, ["turn on 0,0 through 999,999"], 1_000_000) &&
      perform_test(1, ["toggle 0,0 through 999,0"], 1_000) &&
      perform_test(1, ["turn on 0,0 through 999,999","toggle 0,0 through 999,0"], 999_000)
      perform_test(1, ["turn_on 0,0 through 999,999","turn off 499,499 through 500,500"], 999_996)
      perform_test(2, ["turn on 0,0 through 0,0"], 1) &&
      perform_test(2, ["toggle 0,0 through 999,999"], 2_000_000)
    end
  end
end
